...
from z3 import *
# flatten nested lists; chain(*[[a, b], [c, d], ...]) == [a, b, c, d, ...]
...
rows = 'ABCDEFGHI'
cols = '123456789'
boxes = [[Int("{}{}".format(r, c)) for c in cols]
...
s_solver.add(*[Distinct([boxes[i + ii][j + jj] for ii in range(3)
                         for jj in range(3)]) for j in range(0, 9, 3) for i in range(0, 9, 3)])
...
s_solver.add(*[boxes[i][j] == board[i][j] for i in range(9)
               for j in range(9) if board[i][j] != 0])

start = timer()
assert s_solver.check(
) == sat, "Uh oh. The solver didn't find a solution. Check your constraints."
end = timer()
print("\n")
print("Solution found in: {:.2f} milliseconds".format(
    (end - start)*1000), "\n")
